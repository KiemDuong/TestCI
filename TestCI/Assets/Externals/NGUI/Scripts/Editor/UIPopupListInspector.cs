...
public class UIPopupListInspector : Editor
{
	...
	// April 3, NHPhuong added
	void OnSeperator (string spriteName)
	{
		RegisterUndo();
		mList.seperatorSprite = spriteName;
		Repaint();
	}
	...
	public override void OnInspectorGUI ()
	{
		...
		if (mList.atlas != null)
		{
			...
			NGUIEditorTools.SpriteField("Seperator", mList.atlas, mList.seperatorSprite, OnSeperator);
			float lineWidth = EditorGUILayout.FloatField("Line Width", mList.seperatorWidth);
			if (mList.seperatorWidth != lineWidth)
			{
				RegisterUndo();
				mList.seperatorWidth = lineWidth;
			}
			...
			bool exclude = EditorGUILayout.Toggle("Exclude Selection", mList.isExcludeSelection);
			if (mList.isExcludeSelection != exclude )
			{
				RegisterUndo();
				mList.isExcludeSelection = exclude;	
			}
			EditorGUIUtility.LookLikeControls(80f);
			...
			// April 3, 2013 - NHPhuong added
			bool isCenterItems = EditorGUILayout.Toggle("Center Items", mList.isCenterItems);
			...
			bool isClosedWithFoldAnim = EditorGUILayout.Toggle("Fold closing", mList.isClosedWithFoldAnim);
			GUILayout.EndHorizontal();
			...
			if (mList.textScale != ts ||
				...
				mList.isCenterItems != isCenterItems || 
				mList.isClosedWithFoldAnim != isClosedWithFoldAnim )
			{
				RegisterUndo();
				mList.textScale = ts;
				mList.textColor = tc;
