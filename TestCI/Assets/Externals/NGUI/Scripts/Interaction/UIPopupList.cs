...
public class UIPopupList : MonoBehaviour
{
	...
	/// April 03, 2013 - NHPhuong added
	/// <summary>
	/// Name of the seperator sprite.
	/// </summary>
	
	public string seperatorSprite;
	
	/// April 04, 2013 - NHPhuong added
	/// <summary>
	/// Width of the seperator line.
	/// </summary>
	/// 
	public float seperatorWidth = 0.0f;
	...
	// April 3, 2013 - NHPhuong added
	/// <summary>
	/// Popup items will be aligned center.
	/// </summary>
	
	public bool isCenterItems = false;
	...
	/// <summary>
	/// Whether the popup list is closed with folding animation - Not support for position is Position.Above
	/// </summary>
	
	public bool isClosedWithFoldAnim = false;
	...
	// April 4, 2013 - NHPhuong added
	/// <summary>
	/// Exclude the current selected item from the item list
	/// </summary>
	public bool isExcludeSelection = false;
	...
	void Highlight (UILabel lbl, bool instant)
	{
		...
		if (mHighlight != null)
		{
			...
			//April 3, 2013 - NHPhuong removed, this is so tricky!!!!
			float offsetX = 0; //sp.inner.xMin - sp.outer.xMin;
	...
	//LTT: expose this function to public to collapse it's list in some special case
			...
			if ( mChild != null) {
				...
				if (isAnimated)
				{
					...
					if (position == Position.Auto)
					{
						...
						if (cam != null)
						{
							Vector3 viewPos = cam.cachedCamera.WorldToViewportPoint(transform.position);
					...
					for (int i = 0, imax = widgets.Length; i < imax; ++i)
					{
						...
						// April 5, 2013 - NHPhuong added
						if (isClosedWithFoldAnim && !placeAbove && w == mBackground) continue;
					...
					// April 5, 2013 - NHPhuong added
					if (isClosedWithFoldAnim && !placeAbove)
					{
						AnimateScale(mBackground, placeAbove, 0, true);
					}
	...
	// April 5, 2013 - NHPhuong updated: reverse anim
	void AnimateScale (UIWidget widget, bool placeAbove, float bottom, bool reverse = false)
	{
		GameObject go = widget.gameObject;
		Transform t = widget.cachedTransform;
		float minSize = font.size * textScale + mBgBorder * 2f;
		
		float from = reverse ? t.localScale.y : minSize;
		float to = reverse ? minSize : t.localScale.y;

		Vector3 scale = new Vector3(t.localScale.x, to, t.localScale.z);
		t.localScale = new Vector3(t.localScale.x, from , t.localScale.z);
		TweenScale.Begin(go, animSpeed, scale).method = UITweener.Method.EaseOut;

		if (placeAbove)
		{
			Vector3 pos = t.localPosition;
			Vector3 posTo = reverse ? new Vector3(pos.x, pos.y - scale.y + minSize, pos.z) : pos;
			Vector3 posFrom = reverse ? pos : new Vector3(pos.x, pos.y - scale.y + minSize, pos.z);
			
			t.localPosition = posFrom;
			TweenPosition.Begin(go, animSpeed, posTo).method = UITweener.Method.EaseOut;
		}
	}
	...
	void OnClick()
	{
		...
		if (mChild == null && atlas != null && font != null && items.Count > 0)
		{
			...
			// April 3, 2013 - NHPhuong changed: why padding???
...
//			mBackground.cachedTransform.localPosition = new Vector3(0f, bgPadding.y, 0f);
			...
			mHighlight.pivot = isCenterItems ? UIWidget.Pivot.Top : UIWidget.Pivot.TopLeft;
			...
			// April 3, 2013 - NHPhuong changed: offset with bg padding
			float x = 0f, y = -bgPadding.y - padding.y;
//			float x = 0f, y = -padding.y;
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				...
				if (isExcludeSelection && !isFound && mSelectedItem == s) 
				{
					isFound = true;
					continue;
				}
				...
				// April 3, 2013 - NHPhuong added
				lbl.pivot = isCenterItems ? UIWidget.Pivot.Top : UIWidget.Pivot.TopLeft;
//				lbl.pivot = UIWidget.Pivot.TopLeft;
				...
				// June 12, 2013 - DoanDu changed
				//lbl.cachedTransform.localPosition = new Vector3(bgPadding.x + padding.x, y, -0.01f);
				lbl.cachedTransform.localPosition = new Vector3(bgPadding.x + padding.x, y, t.localPosition.z - 10f);
				...
				// April 3, 2013 - NHPhuong removed: do this after making all labels center 
...
//				if (mSelectedItem == s) Highlight(lbl, true);
			...
			Vector3 bcCenter = new Vector3(isCenterItems ? 0 : (x * 0.5f) / fontScale, -0.5f, 0f);
			Vector3 bcSize = new Vector3(x / fontScale, (fontScale + padding.y) / fontScale, 1f);

			// Run through all labels and add colliders, center labels or add seperator lines if needed
			List<UISprite> seperatorLines = new List<UISprite>();
			for (int i = 0, imax = labels.Count; i < imax; ++i)
			{
				UILabel lbl = labels[i];
				
				// April 3, NHPhuong added: center labels
				if (isCenterItems)
				{
					Vector3 pos = lbl.cachedTransform.localPosition;
					lbl.cachedTransform.localPosition = new Vector3(x / 2 + (bgPadding.x + padding.x), pos.y, pos.z);	
				}
				
				// April 3, NHPhuong added: do this after all labels are center
				// Move the selection here if this is the right label
				if (!isExcludeSelection && mSelectedItem == items[i]) Highlight(lbl, true);
				
				// April 3, NHPhuong added: seperator line
				if (seperatorSprite != null && i > 0)
				{
					UISprite seperator = NGUITools.AddSprite(mChild, atlas, seperatorSprite);
					seperator.pivot = lbl.pivot;
					
					UIAtlas.Sprite sp = mHighlight.GetAtlasSprite();
					float offsetX = 0;
					float offsetY = (sp.inner.yMin - sp.outer.yMin + padding.y) / 2;
					
					// June 12, 2013 - DoanDu changed
					float offsetZ = -lbl.cachedTransform.localPosition.z - 10;
					Vector3 pos = lbl.cachedTransform.localPosition + new Vector3(-offsetX, offsetY, offsetZ);
					//Vector3 pos = lbl.cachedTransform.localPosition + new Vector3(-offsetX, offsetY, 0f);
					
					seperator.cachedTransform.localPosition = pos;
					seperator.cachedTransform.localRotation = Quaternion.identity;
					seperator.cachedTransform.localScale = new Vector3(seperatorWidth == 0.0f ? x : seperatorWidth , 5f, 1f);
					
					seperatorLines.Add(seperator);
				}
			...
			// April 3, 2013 - NHPhuong changed: should be bottom padding
			y -= bgPadding.w;
//			y -= bgPadding.y;
			
			// Scale the background sprite to envelop the entire set of items
			// April 3, 2013 - NHPhuong changed
			mBackground.cachedTransform.localScale = new Vector3(x, -y, 1f);
//			mBackground.cachedTransform.localScale = new Vector3(x, -y + bgPadding.y, 1f);
			...
			// April 4, 2013 - NHPhuong added
			if (isExcludeSelection) mHighlight.gameObject.SetActive(false);
			...
			if (isAnimated)
			{
				...
				// April 4, 2013 - NHPhuong added
				if (!isExcludeSelection) Animate(mHighlight, placeAbove, bottom);
//				Animate(mHighlight, placeAbove, bottom);
				
				for (int i = 0, imax = labels.Count; i < imax; ++i) Animate(labels[i], placeAbove, bottom);
				
				// April 4, 2013 - NHPhuong added
				if (seperatorSprite != null) 
				{
					for (int i = 0, imax = seperatorLines.Count; i < imax; i++) 
						Animate(seperatorLines[i], placeAbove, bottom);	
				}
